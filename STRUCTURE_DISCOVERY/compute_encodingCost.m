...
switch subgraph
    ...
    case 'bc'
        ...
        l = n_sub2;
        if E(1) == 0 || E(2) == 0 %if sum(sum(E)) == 0
            MDLcost = LN(k) + LN(l) + l2cnk(N_tot, k) + l2cnk(N_tot-k, l); %log2( nchoosek(vpi(N_tot), k) ) + log2( nchoosek(vpi(N_tot-k), l) );
        else
            MDLcost = LN(k) + LN(l) + l2cnk(N_tot, k) + ...            %log2( nchoosek(vpi(N_tot), k) ) + ...
                l2cnk(N_tot-k, l)        + ...      %log2( nchoosek(vpi(N_tot-k), l) ) + ...
                Lnu_opt( E );
        end
    case 'nb'
        k = n_sub;
        ...
        if E(1) == 0 || strcmp('ca', true) || E(2) == 0 %if sum(sum(E)) == 0
            ...
            if edges_inc == 0 || edges_exc == 0
                MDLcost = LN(k) + LN(l) + l2cnk(N_tot, k) + l2cnk(N_tot-k, l) +  Lnu_opt( E);  ... %log2( nchoosek(vpi(N_tot), k) ) + ...
...
%% Alternative error per structure: Naive Data-to-Model
    function [ c_err ] = Lnd( E )
        Einc = nnz(E);
        c_err = LN( Einc ) + l2cnk (N_tot^2, Einc);   %log2( nchoosek(vpi(N_tot^2), Einc) );
    end
